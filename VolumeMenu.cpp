...
#pragma region VolumeOptions
	...
	UIText* solid = new UIText("Solid", 40.0f, osgText::TextBase::CENTER_CENTER);//GA
	solid->setColor(osg::Vec4(1, 0.8, 0.8, 1));

	_blacknwhite->addChild(bnw);
	_rainbow->addChild(rnbw);
	_solid->addChild(solid);

	list2->addChild(_blacknwhite);
	list2->addChild(_rainbow);
	list2->addChild(_solid);
	list->addChild(list2);
...
#pragma endregion
	...
	if (_volume->hasMask())
	{
		...
		cpHLabel->setPercentPos(osg::Vec3(0, 0, -.5));//check
		...
		_maskMenu->getRootElement()->setAbsoluteSize(ConfigManager::getVec3("Plugin.HelmsleyVolume.Orientation.MaskMenu.Scale", osg::Vec3(500, 1, 800)));//GA X was 400
		...
		_colorMenu->getRootElement()->setAbsoluteSize(ConfigManager::getVec3("Plugin.HelmsleyVolume.Orientation.MaskMenu.Scale", osg::Vec3(500, 1, 800)));//GA X was 400

		list = new UIList(UIList::TOP_TO_BOTTOM, UIList::CONTINUE);
		list->setPercentPos(osg::Vec3(0, 0, -0.2));
		list->setPercentSize(osg::Vec3(.5, 1, 0.8));//GA X was 1
		bknd->addChild(list);
		...
		UIQuadElement* bodyColorButton = new UIQuadElement(osg::Vec4(0, 0, 0, 1));
		_bodyColorButton = bodyColorButton;
		_bodyCol = osg::Vec3(0, 0, 100);
		cp->setButton(_bodyColorButton);
		

		bodyColorButton->addChild(_bodyColCallback);
		bodyColorButton->setAbsoluteSize(osg::Vec3(100, 0.1, 100));
		bodyColorButton->setPercentSize(osg::Vec3(0, 1, 0));
		bodyColorButton->setAbsolutePos(osg::Vec3(-100, -0.15f, -200));
		bodyColorButton->setPercentPos(osg::Vec3(1, 0, 0));
		...
		if (!_movable)
		{
			...
			//_colorMenu->setActive(true, true);
		}
		else {
			_maskMenu->setActive(true, false);
			//_colorMenu->setActive(true, false);
...
{	
	//<--------------------------------------------------COLORED BUTTONS
	if (item == _bodyColCallback) {
		_cp->setSaveColor(&_bodyCol);
		_cp->setCPColor(_bodyCol);
		colorButtonPress(_bodyColorButton, organRGB(BODY), "Body", _organs);
		
	}

	//colorButtonPress(cvr::UIQuadElement* button, organRGB organRGB, std::string organName,VisibilityToggle* organEye)
	...
	{
		_volume->getCompute()->getOrCreateStateSet()->setDefine("BODY", _organs->isOn());
	...
	else if (item == _transferFunction)
	{
		...
		//GA
		else if (_transferFunction->getCurrent() == 2)
		{
			...
			((UIText*)_blacknwhite->getChild(0))->setColor(UI_INACTIVE_COLOR);
			((UIText*)_rainbow->getChild(0))->setColor(UI_INACTIVE_COLOR);
			((UIText*)_solid->getChild(0))->setColor(UI_ACTIVE_COLOR);
